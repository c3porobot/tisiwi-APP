...
{
    return self.arrow.arrowImageName;
}
...
- (void)setArrowImageName:(NSString *)arrowImageName
{
    ...
    self.arrow.frame = CGRectMake(0, (SVPullToRefreshViewHeight - (image.size.height / [UIScreen mainScreen].scale)) / 2.0f, (image.size.width / [UIScreen mainScreen].scale), (image.size.height / [UIScreen mainScreen].scale));
    
    self.arrow.arrowImageName = arrowImageName;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.arrowImageName) {
        [[UIImage imageNamed:self.arrowImageName] drawInRect:rect];
